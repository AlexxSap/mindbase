...
Преимущество `std::span` заключается в том, что он содержит информацию о количестве элементов данных. Таким образом, не нужен дополнительный параметр под размер массива. Еще можно сделать его из подпоследовательности.
